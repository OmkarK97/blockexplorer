...
function App() {
  ...
  const Block = () => {
    return (
      <>
        ...
        >
          <button style={{ marginRight: 24 }} onClick={() => previousBlock()}>
            Previous Block
          </button>
          <p>{` Block Number: ${blockNumber} `}</p>
          <button style={{ marginLeft: 24 }} onClick={() => nextBlock()}>
            Next Block
          </button>
        </div>
      </>
              ...
              {blockTransactions.map((transaction) => {
                return (
                  ...
                  >
                    ...
                    <th>
                      {parseFloat(
                        Utils.formatEther(transaction.value.toString())
                      ).toFixed(12)}
                    </th>
  ...
  const Detail = (props) => {
    return (
      ...
      >
        <p style={{ marginLeft: 10, marginRight: 10, fontWeight: "bold" }}>
          {props.name}:
        </p>
        <p>{props.value}</p>
      </div>
  ...
  const TransactionDetail = (props) => {
    return (
      <>
        ...
        <Detail
          name={"Confirmations"}
          value={props.transaction.confirmations}
        />
        <Detail
          name={"value"}
          value={
            props.transaction?.value
              ? parseFloat(
                  Utils.formatEther(props.transaction?.value.toString())
                )
              : "0"
          }
        />
        <Detail
          ...
          value={calcFee(props.transaction, 18)}
        />
        <div
          style={{
            display: "flex",
